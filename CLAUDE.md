...
- **Cache-Busting (`d33`; Commits `9f1ef27` Code, `fd3b01b` Doku):** JS/CSS-Assets tragen jetzt `?v=<ASSET_VERSION>` via zentralem `asset()`-Jinja-Helper (`app/frontend/deps.py`) + Setting `asset_version` (`config.py`). `Dockerfile` ARG `ASSET_VERSION` (Default `dev`), CI (`docker-publish.yml`) setzt den **Build-Timestamp** (kein Git-SHA → kein Commit-Leak im HTML). 6 Templates umgestellt; **Fonts bewusst un-versioniert** (Preload/`url()`-Mismatch → Doppellading). Deploy-Runbook im README (`pull → up -d → grep version → curl ?v= → CF-Purge`), Fallstricke in `docs/lessons-learned.md`. Verifiziert: ruff+mypy+76 Frontend-Unit-Tests grün, isolierter Render-Test (Default/ENV-Override/6 Templates). **Offen:** scharfe Prod-Verifikation (greift `?v=`-Bust am CF-Edge?) erst beim **nächsten Image-Build** mit CI-Timestamp; dann einmalig prüfen, dass **Cloudflare-Caching-Level „Standard"** ist (nicht „Ignore Query String"), sonst greift der Edge-Bust nicht.
