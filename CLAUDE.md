...
- **`m4ct` (feature P3, `ae78217`, closed):** Globales ASGI-Request-Body-Limit (Defense-in-Depth zu `fbq`). Pure-ASGI `RequestBodySizeLimitMiddleware` (`app/security/middleware.py`), als **äußerste** Middleware registriert (vor CSRF). Content-Length-Fast-Path + Byte-Counter (chunked/gefälscht) → 413. `MAX_REQUEST_BODY_MB` (Default 1024, 0=off). 8 Unit-Tests + Funktionstest gegen echte App (413 vor CSRF). Follow-up `wr2s` (Proxy-Limit) angelegt.
- **`q9td` (feature P3, `cd7cec6`, closed) — LIVE abgenommen:** Harter Per-Datei-Timeout im Preview-Worker via **pebble `ProcessPool`** (neue Dependency, `spawn`). `asyncio.to_thread` bot keinen echten Timeout (Threads nicht killbar); pebble killt hängende Worker-Prozesse hart + Auto-Restart. `app/images/process_pool.py` (`get_pool`/`shutdown_pool`/`run_in_pool`), picklebare bytes-Wrapper `render_*_bytes`. `IMAGE_PROCESSING_TIMEOUT_SECONDS`=60 (0=off), `IMAGE_PROCESSING_WORKERS`=2. Pool-Teardown im lifespan. 7 Unit-Tests. **Live am lokalen Stack: 12er-Multi-Upload → 0.08s (kein Freeze) → alle ready, 36 Previews, kein Crash.**
- **`e5n` (task P3, `4eb4a2b` + `212c1b0` Format-Fix, closed):** Bild-Zählung via `func.count()` statt `len(scalars().all())` an 5 Stellen (`images/router.py`, `frontend/galleries.py`, `guest/router.py` 3×). Härtet zugleich die unauth. Guest-Pfade gegen Memory-Exhaustion. Integration-Tests gegen lokales Postgres grün.
- **`bkw` (task P3, `e7312cb` + `f0a1e34`, closed) — LIVE abgenommen:** Zwei DRY-Konsolidierungen. (3.2) `ALLOWED_TRANSITIONS` einmalig in `app/db/models.py` (beide Router importieren). (2.2) Frontend-`delete_gallery` nutzt jetzt `purge_gallery` statt duplizierter Lösch-Logik (DSGVO: keine verwaisten Bilddateien). Neuer Integration-Test (`test_frontend_gallery_delete.py`): Storage-Stub beweist Löschung aller Keys. **Live am Stack mitbewiesen (Frontend-Delete der Test-Galerie).**
- **`vblf` (task P3, `3468981`, closed) — LIVE abgenommen:** Zentrale Logging-Config (`app/logging_config.py`, `configure_logging()`), aktiviert in `main.py` (+ Re-Apply nach `run_migrations`, da Alembics `fileConfig` das Logging überschreibt) + ProcessPool-`initializer`. `LOG_LEVEL` (Default INFO). App-`INFO`-Logs vorher unsichtbar (lastResort-Handler) → jetzt sichtbar im eigenen Format, uvicorn-Logs erhalten, kein Doppel-Logging. 6 Unit-Tests. **Live am Stack: Worker-Logs + uvicorn-Access erscheinen, kein Doppel.** Zwei Fallstricke dokumentiert (`docs/lessons-learned.md`).
- **Stand Tests/Doku:** 344 Unit-Tests grün, ruff+format+mypy sauber, CHANGELOG (Security + Changed) + `.env.example` + lessons-learned aktuell. **Alle Backend/Observability — Kapitän deployt/nimmt gegen Prod ab.**

### Erledigt in der Wache davor (2026-06-16) — Cleanup + 2 User-Findings + 2 externe Security-Reviews + 3 P2-Härtungen
