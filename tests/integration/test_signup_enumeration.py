...
async def test_fresh_signup_creates_pending_and_returns_neutral(client, db):
    resp = await client.post(
        "/api/v1/auth/signup",
        json={"email": "fresh@test.local", "password": "correct horse"},
    )
    ...
    assert await _pending_count(db, "fresh@test.local") == 1
...
async def test_signup_existing_user_is_neutral_and_idempotent(client, db):
    ...
    existing = await make_user(db, "taken@test.local", status=UserStatus.active)
    original_hash = existing.password_hash

    resp = await client.post(
        "/api/v1/auth/signup",
        json={"email": "taken@test.local", "password": "attacker-chosen-pw"},
    )
    ...
    assert await _pending_count(db, "taken@test.local") == 0
...
async def test_signup_existing_pending_is_neutral_and_not_overwritten(client, db):
    ...
    resp = await client.post(
        "/api/v1/auth/signup",
        json={"email": "pending@test.local", "password": "second-attempt-pw"},
    )
    ...
    assert await _pending_count(db, "pending@test.local") == 1
    result = await db.execute(select(PendingSignup).where(PendingSignup.email == "pending@test.local"))
...
async def test_signup_short_password_still_rejected(client, db):
    ...
    assert await _pending_count(db, "shortpw@test.local") == 0
