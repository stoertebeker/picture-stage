...
def test_skip_link_on_all_standalone_pages():
    ...
    landmark. Covers base/guest_base (inherited by all pages) and the shared
    auth_base layout. Auth + setup pages extend auth_base and inherit the skip
    link — verified separately below."""
    pages = [
        ("base.html",),
        ("guest_base.html",),
        ("auth_base.html",),
    ...
    # Auth + setup pages inherit the skip link by extending auth_base.
    for child_parts in [
        ("auth", "login.html"),
        ("auth", "signup.html"),
        ("auth", "verify.html"),
        ...
        html = (PROJECT_ROOT / "app" / "templates").joinpath(*child_parts).read_text()
        assert 'extends "auth_base.html"' in html, f"{child_parts} must extend auth_base for the skip link"
